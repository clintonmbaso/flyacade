  ...
  questions.forEach((q, index) => {
    ...
    if (!addedSections[q.type]) {
      ...
      switch (q.type) {
        ...
        case "unique_special":
          sectionIdentifierText = "Unique Paper 2 Section";
          break;
        
        case "oral":
          sectionIdentifierText = "Oral Section";
          break;
        ...
        case "sounds":
          sectionIdentifierText = "Sounds Section";
          break;
        ...
        case "spellings_unique":
          sectionIdentifierText = "Spellings Section.";
          break;
        
        case "punctuations_unique":
          sectionIdentifierText = "Punctuations Section.";
          break;
        
        case "wordMeaning_unique":
          sectionIdentifierText = "Word Meaning Section.";
          break;
        
        case "sentenceOrdering_unique":
          sectionIdentifierText = "Sentence Ordering Section.";
          break;
        ...
        case "sorting":
          sectionIdentifierText = "Sorting Section";
          break;        
...
case "oral":
  const oralQuestionWrapper = document.createElement("div");
  
  // Create a label for the oral question
  const oralQuestionLabel = document.createElement("label");
  oralQuestionLabel.innerHTML = `Oral Question: ${questionText}`;
  oralQuestionWrapper.appendChild(oralQuestionLabel);
  oralQuestionWrapper.appendChild(document.createElement("br"));
  
  // Create a prompt to guide the student
  const oralPrompt = document.createElement("p");
  oralPrompt.innerHTML = "Please answer the following question orally:";
  oralQuestionWrapper.appendChild(oralPrompt);
  
  // Add an input field for the teacher to type their feedback or track completion
  const feedbackInput = document.createElement("textarea");
  feedbackInput.setAttribute("id", `feedback-${index}`);
  feedbackInput.setAttribute("name", `feedback-${index}`);
  feedbackInput.setAttribute("placeholder", "Teacher's feedback or notes...");
  oralQuestionWrapper.appendChild(feedbackInput);
  oralQuestionWrapper.appendChild(document.createElement("br"));

  // Create a button to simulate a recording or verbal confirmation (could be a real feature with more setup)
  const recordButton = document.createElement("button");
  recordButton.innerHTML = "Start Recording";
  recordButton.setAttribute("id", `record-btn-${index}`);
  
  // Placeholder action for the record button
  recordButton.onclick = function() {
    alert("This would start the recording process in a real application.");
  };
  
  oralQuestionWrapper.appendChild(recordButton);

  // Create a submit button for teachers to acknowledge the response (even without recording)
  const submitButton = document.createElement("button");
  submitButton.innerHTML = "Submit Answer";
  submitButton.setAttribute("id", `submit-btn-${index}`);
  
  // Placeholder for submitting answer
  submitButton.onclick = function() {
    alert("Oral response submitted!");
  };

  oralQuestionWrapper.appendChild(submitButton);
  questionWrapper.appendChild(oralQuestionWrapper);
  break; 
...
case "sounds":
  ...
  // Function to group questions by sound
  function groupQuestionsBySound(questions) {
    const groupedQuestions = {};

    questions.forEach((q) => {
      if (!groupedQuestions[q.sound]) {
        groupedQuestions[q.sound] = [];
      }
      groupedQuestions[q.sound].push(q);
    });

    return groupedQuestions;
  }

  // Function to get questions based on category distribution
  function getDistributedQuestions(groupedQuestions, totalQuestions) {
    const soundCategories = Object.keys(groupedQuestions);
    const questionsPerCategory = Math.floor(totalQuestions / soundCategories.length);
    let extraQuestionsNeeded = totalQuestions % soundCategories.length;

    const selectedQuestions = [];

    // First, distribute questions evenly
    soundCategories.forEach((sound) => {
      const soundQuestions = groupedQuestions[sound];
      // Select the required number of questions from this sound category
      const questionsToPick = soundQuestions.slice(0, questionsPerCategory);
      selectedQuestions.push(...questionsToPick);
    });

    // Then, if any category should get an extra question, add one to the category with the longest sound
    if (extraQuestionsNeeded > 0) {
      // Sort categories by sound length (longer sound gets the extra question)
      const sortedCategories = soundCategories.sort((a, b) => b.length - a.length);
      const longestSound = sortedCategories[0];
      const soundQuestions = groupedQuestions[longestSound];
      // Add the extra question to this category
      selectedQuestions.push(soundQuestions[questionsPerCategory]);
    }
  ...
  // Group questions by sound
  const groupedQuestions = groupQuestionsBySound(questions);

  // Total questions needed
  const totalQuestions = 4; // Define how many questions you need

  // Get the questions based on the distribution
  const distributedQuestions = getDistributedQuestions(groupedQuestions, totalQuestions);

  // Display the selected random questions on the page
  distributedQuestions.forEach((q, index) => {
    const questionWrapper = document.createElement("div");
    const questionText = document.createElement("p");
    questionText.innerHTML = q.question;
    questionWrapper.appendChild(questionText);

    // Create the input field for the sound question
    const soundInput = document.createElement("input");
    soundInput.setAttribute("type", "text");
    soundInput.setAttribute("id", `sound-answer-${index}`);
    soundInput.setAttribute("name", `sound-question-${index}`);
    const soundLabel = document.createElement("label");
    soundLabel.innerHTML = "Sound: ";
    questionWrapper.appendChild(soundLabel);
    questionWrapper.appendChild(soundInput);
    questionWrapper.appendChild(document.createElement("br"));
...
case "number_comparison":
    // Create the main container for number comparison
    const numCompareWrapper = document.createElement("div");
    numCompareWrapper.classList.add("num-compare-wrapper");


    // Create a box to hold numbers (without operator)
    const numCompareBox = document.createElement("div");
    numCompareBox.classList.add("num-compare-box");

    const numLeft = document.createElement("div");
    numLeft.classList.add("num-compare-value");
    numLeft.textContent = q.comparison1;
    numCompareBox.appendChild(numLeft);

    // Input field for the user to enter the operator
    const operatorInput = document.createElement("input");
    operatorInput.setAttribute("type", "text");
    operatorInput.setAttribute("maxlength", "1"); // Only allow one character
    operatorInput.setAttribute("id", `operator-answer-${q.comparison1}-${q.comparison2}`);
    operatorInput.setAttribute("name", `operator-question-${q.comparison1}-${q.comparison2}`);
    operatorInput.classList.add("num-compare-input");
    operatorInput.placeholder = "?"; // Placeholder to indicate input is needed
    numCompareBox.appendChild(operatorInput);

    const numRight = document.createElement("div");
    numRight.classList.add("num-compare-value");
    numRight.textContent = q.comparison2;
    numCompareBox.appendChild(numRight);

    numCompareWrapper.appendChild(numCompareBox);

    // Append to the main question wrapper
    questionWrapper.appendChild(numCompareWrapper);
    break;
...
case "sentenceOrdering_unique":
  const sentenceOrderingContainer = document.createElement("div");
  sentenceOrderingContainer.classList.add("sentence-ordering-container");

  if (q.format === "dragdrop") {
    // Drag-and-Drop Sentence Ordering
    const sentenceContainer = document.createElement("div");
    sentenceContainer.classList.add("sortable-container");
    ...
    shuffledWords.forEach((word, i) => {
      ...
      wordDiv.setAttribute("id", `word-${index}-word-${i}`);
      wordDiv.setAttribute("class", "sortable-item");
      wordDiv.setAttribute("draggable", "true");
      wordDiv.textContent = word;

      wordDiv.addEventListener("dragstart", (e) => {
        e.dataTransfer.setData("text", wordDiv.id);
      });

      sentenceContainer.appendChild(wordDiv);
    });

    // Make the sentence container a drop zone
    ...
    sentenceOrderingContainer.appendChild(sentenceContainer);
  }

  if (q.format === "multiple_choice") {

    // Shuffle and display the answer choices
    const shuffledChoices = [...q.choices].sort(() => Math.random() - 0.5);

    const sentenceChoicesContainer = document.createElement("div");
    sentenceChoicesContainer.classList.add("sentence-choices-container");

    shuffledChoices.forEach((choice, index) => {
      const choiceLabel = document.createElement("label");
      const choiceInput = document.createElement("input");
      choiceInput.setAttribute("type", "radio");
      choiceInput.setAttribute("name", `sentence-order-${index}`);
      choiceInput.setAttribute("value", choice);
      choiceLabel.appendChild(choiceInput);
      choiceLabel.appendChild(document.createTextNode(` ${choice}`));
      sentenceChoicesContainer.appendChild(choiceLabel);
      sentenceChoicesContainer.appendChild(document.createElement("br"));
    });

    // Append choices container
    sentenceOrderingContainer.appendChild(sentenceChoicesContainer);
  }


          // Append the math container to the question wrapper
    questionWrapper.appendChild(sentenceOrderingContainer);
    break;
...
case "image_based":
    ...
    imagesContainer.classList.add("images-container"); // Optional class for styling

    // Ensure q.imageSrc is an array (supporting multiple images)
    if (Array.isArray(q.imageSrc) && q.imageSrc.length > 0) {
        q.imageSrc.slice(0, 5).forEach((imgSrc, imgIndex) => {
            if (imgSrc) {
                console.log(`Loading image ${imgIndex + 1} from: ${imgSrc}`); // Debugging log
                const imgElement = document.createElement("img");
                imgElement.setAttribute("src", imgSrc);
                imgElement.setAttribute("alt", `Question Image ${imgIndex + 1}`);
                imgElement.setAttribute("width", "150px");
                imgElement.setAttribute("height", "auto");
                imagesContainer.appendChild(imgElement);
            } else {
                console.log(`Image source missing for image ${imgIndex + 1} in question: `, q);
            }
        });
    }

    // Append images container if it has images
    if (imagesContainer.children.length > 0) {
        imageQuestionContainer.appendChild(imagesContainer);
        imageQuestionContainer.appendChild(document.createElement("br"));
    }

    // Add a question description below the images
    ...
    // Based on the sub-question type, add multiple-choice or fill-in-the-blank options
    ...
    imgElement.setAttribute("width", "150px"); // Set width as per your requirement
...
case "imaging":
    ...
    imagingImgElement.setAttribute("width", "150px"); // Set width as needed
...
case "sorting":
  ...
  sortingContainer.appendChild(questionElement);

  // Create the sortable items (this can include text, numbers, or images)
  const shuffledItems = [...q.items].sort(() => Math.random() - 0.5);
  
  shuffledItems.forEach((item, i) => {
    const itemDiv = document.createElement("div");
    itemDiv.setAttribute("id", `item-${index}-item-${i}`);
    itemDiv.setAttribute("class", "sortable-item");
    itemDiv.setAttribute("draggable", "true");

    // If the item is an image, create an img element
    if (item.type === "image") {
      const imgElement = document.createElement("img");
      ...
      // Otherwise, it's a text item or a number
      itemDiv.textContent = item.name;
    }

    // Add a dragstart event listener
    itemDiv.addEventListener("dragstart", (e) => {
      e.dataTransfer.setData("text", itemDiv.id);
    });

    sortingContainer.appendChild(itemDiv);
  });

  // Create container for sorted items based on the number of groups
  const groups = q.groups; // Assume q.groups is an array of group names (e.g., ["Living", "Non-Living"] or ["Order"])

  groups.forEach((group, groupIndex) => {
    const sortedContainer = document.createElement("div");
    sortedContainer.setAttribute("id", `sorted-container-${index}-group-${groupIndex}`);
    sortedContainer.setAttribute("class", "sorted-container");
    const groupTitle = document.createElement("h4");
    groupTitle.textContent = group;
    sortedContainer.appendChild(groupTitle);

    // Add a dragover event to allow dropping
    sortedContainer.addEventListener("dragover", (e) => {
      e.preventDefault();
    });

    // Add a drop event to append the dragged item to the corresponding sorted container
    sortedContainer.addEventListener("drop", (e) => {
      const draggedItemId = e.dataTransfer.getData("text");
      const draggedItem = document.getElementById(draggedItemId);
      // If the question involves ordering numbers, we can check and place items in order
      if (q.type === "order") {
        // Custom logic for sorting numbers
        const itemsInOrder = Array.from(sortedContainer.children);
        const draggedItemNumber = parseFloat(draggedItem.textContent); // or item.value if using a different type
        let inserted = false;

        // Sort items in ascending or descending order based on the type
        itemsInOrder.forEach((existingItem, i) => {
          const existingItemNumber = parseFloat(existingItem.textContent);
          if (!inserted && ((q.order === "ascending" && draggedItemNumber < existingItemNumber) || 
                             (q.order === "descending" && draggedItemNumber > existingItemNumber))) {
            sortedContainer.insertBefore(draggedItem, existingItem);
            inserted = true;
          }
        });

        if (!inserted) {
          sortedContainer.appendChild(draggedItem); // Append at the end if it's the largest or smallest
        }
      } else {
        // For general sorting, just append the dragged item
        sortedContainer.appendChild(draggedItem);
      }
    });

    // Append each sorted container to the sorting container
    sortingContainer.appendChild(sortedContainer);
  });

  // Add the sorting container to the question wrapper
  questionWrapper.appendChild(sortingContainer);
  break;
...
case "punctuations_unique":
  // Punctuation Quiz - Multiple Choice (Unique)
  const punctuationQuestionContainer = document.createElement("div");
  punctuationQuestionContainer.classList.add("punctuation-question-container");
      
  // Shuffle and display the answer choices
  const shuffledPunctuationChoices = [...q.choices].sort(() => Math.random() - 0.5);
  
  const punctuationChoicesContainer = document.createElement("div");
  punctuationChoicesContainer.classList.add("punctuation-choices-container");

  shuffledPunctuationChoices.forEach((choice, index) => {
    const choiceLabel = document.createElement("label");
    const choiceInput = document.createElement("input");
    choiceInput.setAttribute("type", "radio");
    choiceInput.setAttribute("name", `punctuation-question-${index}`);
    choiceInput.setAttribute("value", choice);
    choiceLabel.appendChild(choiceInput);
    choiceLabel.appendChild(document.createTextNode(` ${choice}`));
    punctuationChoicesContainer.appendChild(choiceLabel);
    punctuationChoicesContainer.appendChild(document.createElement("br"));
  });

  // Append choices container to the main question container
  punctuationQuestionContainer.appendChild(punctuationChoicesContainer);


        // Append the question container to the main English container
  questionWrapper.appendChild(punctuationQuestionContainer);
  break;
    ...
    case "wordMeaning_unique":
      // Word Meaning Quiz (Unique)
      const meaningChoicesContainerUnique = document.createElement("div");
      
      // Shuffle the choices for word meanings
      const shuffledChoicesUnique = [...q.choices].sort(() => Math.random() - 0.5);
      
      shuffledChoicesUnique.forEach((choice, i) => {
        const choiceInputUnique = document.createElement("input");
        choiceInputUnique.setAttribute("type", "radio");
        choiceInputUnique.setAttribute("id", `question-${index}-choice-unique-${i}`);
        choiceInputUnique.setAttribute("name", `question-${index}-unique`);
        choiceInputUnique.setAttribute("value", choice);
        
        const choiceLabelUnique = document.createElement("label");
        choiceLabelUnique.setAttribute("for", `question-${index}-choice-unique-${i}`);
        choiceLabelUnique.innerHTML = choice;
        
        meaningChoicesContainerUnique.appendChild(choiceInputUnique);
        meaningChoicesContainerUnique.appendChild(choiceLabelUnique);
        meaningChoicesContainerUnique.appendChild(document.createElement("br"));
      });
    
      questionWrapper.appendChild(meaningChoicesContainerUnique);
       break;
...
let rows = q.rows || 2; // Number of rows, defaults to 2
    ...
    // Limit the numbers array to maxNumbers
    numbers = numbers.slice(0, maxNumbers);
