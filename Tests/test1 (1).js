...
// Define mark allocations per question for each section type
const markAllocationPerQuestion = {
    multiple_choice: 1,
    special: 2,
    oral: 1,
    true_false: 1,
    ...
    math: 2,
    spellings_unique: 1,
    punctuations_unique: 1,
    wordMeaning_unique: 1,
    sentenceOrdering_unique: 1,
    comprehension: 3,
    image_based: 2,
    imaging: 2,
    maze: 1,
...
// Function to calculate total marks based on number of questions in a section
function calculateTotalMarks(sectionType, totalQuestions) {
    const marksPerQuestion = markAllocationPerQuestion[sectionType] || 0;
    return totalQuestions * marksPerQuestion;
}

// Function to dynamically add the section marks inputs
function addSectionMarksInputs(questionWrapper, sectionType, totalQuestions) {
    // Create container for inputs
    const sectionMarksContainer = document.createElement('div');
    sectionMarksContainer.classList.add('section-marks');
    sectionMarksContainer.style.marginTop = "10px";

    // Create first input (Total Marks - Readonly)
    ...
    totalMarksInput.style.width = "45%";
    totalMarksInput.style.display = "inline-block";
    totalMarksInput.style.marginRight = "5%";
    totalMarksInput.value = calculateTotalMarks(sectionType, totalQuestions); // Set calculated value

    // Create second input (Manual Entry)
    const manualMarksInput = document.createElement('input');
    manualMarksInput.type = "text";
    manualMarksInput.placeholder = "Enter marks per question manually";
    manualMarksInput.style.width = "45%";
    manualMarksInput.style.display = "inline-block";

    // Append inputs to container
    sectionMarksContainer.appendChild(totalMarksInput);
    sectionMarksContainer.appendChild(manualMarksInput);

    // Append the container to the question wrapper
    questionWrapper.appendChild(sectionMarksContainer);
}
/*
// Example: Assuming you have a function that loads questions per section
function loadQuestions(sectionType, questions) {
    const questionWrapper = document.createElement('div');
    questionWrapper.classList.add('question-wrapper');



    // Add section marks inputs
    addSectionMarksInputs(questionWrapper, sectionType, questions.length);

    // Append questionWrapper to the main container (modify as per your structure)
    document.getElementById('main-container').appendChild(questionWrapper);
